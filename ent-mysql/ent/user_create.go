...
func (uc *UserCreate) SetState(es enum.UserState) *UserCreate {
	uc.mutation.SetState(es)
	return uc
}

// SetNillableState sets the "state" field if the given value is not nil.
func (uc *UserCreate) SetNillableState(es *enum.UserState) *UserCreate {
	if es != nil {
		uc.SetState(*es)
	}
	return uc
...
func (uc *UserCreate) defaults() error {
	...
	if _, ok := uc.mutation.UUID(); !ok {
		if user.DefaultUUID == nil {
			return fmt.Errorf("ent: uninitialized user.DefaultUUID (forgotten import ent/runtime?)")
		}
		v := user.DefaultUUID()
